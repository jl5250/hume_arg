...
<head>
    ...
    <script type="text/javascript">
        function changlimit() {
            ...
            location.href = "${path}/user/userorders?page=1&limit=" + limit;
        }
    </script>
</head>
<body>
        ...
        <c:otherwise>
            <table class="table table-hover">
                ...
                <c:forEach begin="0" step="1" items="${userlist}" var="list" varStatus="userlist">
                    ...
                    <tr>
                        ...
                        <td>${list.time}</td>
                        <td>
                            <c:if test="${list.payState == 0 }">
                                <a href="${path }/user/afterpay?orderId=${list.orderId}">
                                    <button type="button" class="btn btn-info btn-sm">支付</button>
                                </a>
                            </c:if>
                        </td>
                    </tr>
            ...
            <ul class="pager">
                <c:choose>
                    ...
                    <c:otherwise>
                        <li><a href="${path}/user/userorders?page=${page.firstPage}&limit=${limit}">首页</a></li>
                        <li><a href="${path}/user/userorders?page=${page.prePage}&limit=${limit}">&laquo;上一页</a></li>
                    </c:otherwise>
                </c:choose>
                <c:choose>
                    <c:when test="${page.pageNum == page.pages }"></c:when>
                    <c:otherwise>
                        <li><a href="${path}/user/userorders?page=${page.nextPage}&limit=${limit}">下一页&raquo;</a></li>
                        <li><a href="${path}/user/userorders?page=${page.lastPage}&limit=${limit}">最后页</a></li>
                    </c:otherwise>
                </c:choose>
