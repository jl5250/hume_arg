...
<head>
    ...
    <script type="text/javascript">
        function changlimit() {
            ...
            location.href = "${path}/root/checkstation?page=1&limit=" + limit;
        }
    </script>
</head>
<body>
<div align="center">
    <c:choose>
        <c:when test="${page.pages == 0 }">
            <h2 style="color: red;">暂时没有新注册的快递站</h2>
        </c:when>
        <c:otherwise>
            <table class="table table-hover">
                ...
                <c:forEach begin="0" step="1" items="${userlist}" var="list" varStatus="userlist">
                    ...
                    <tr>
                        ...
                        <td>${list.area }</td>
                        <td>${list.address }</td>
                        <td><a href="${path }/root/passstation?stationId=${list.stationId}">
            ...
            <ul class="pager">
                <c:choose>
                    ...
                    <c:otherwise>
                        <li><a href="${path}/root/checkstation?page=${page.firstPage}&limit=${limit}">首页</a></li>
                        <li><a href="${path}/root/checkstation?page=${page.prePage}&limit=${limit}">&laquo;上一页</a></li>
                    </c:otherwise>
                </c:choose>
                <c:choose>
                    <c:when test="${page.pageNum == page.pages }"></c:when>
                    <c:otherwise>
                        <li><a href="${path}/root/checkstation?page=${page.nextPage}&limit=${limit}">下一页&raquo;</a></li>
                        <li><a href="${path}/root/checkstation?page=${page.lastPage}&limit=${limit}">最后页</a></li>
                    </c:otherwise>
                </c:choose>
