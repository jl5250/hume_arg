...
<head>
    ...
    <script type="text/javascript">
        function changlimit() {
            ...
            location.href = "${path}/base/searchOrder?page=1&limit=" + limit;
        }
    </script>
</head>
<body>
<div align="center">
    <table class="table table-hover">
        ...
        <c:forEach begin="0" step="1" items="${userlist}" var="list" varStatus="userlist">
            ...
            <tr>
                ...
                <td>${list.senderAddr}</td>
                <td>${list.recipientName}<br>${list.recipientPhone}</td>
                <td>${list.recipientAddr}</td>
                <td>${list.money }</td>
                <td>${list.notes }</td>
                <td>${list.currentProvince}<br>${list.currentCity}<br>
                    <c:choose>
    ...
    <ul class="pager">
        <c:choose>
            ...
            <c:otherwise>
                <li><a href="${path}/base/searchOrder?page=${page.isFirstPage}&limit=${limit}">首页</a></li>
                <li><a href="${path}/base/searchOrder?page=${page.prePage}&limit=${limit}">&laquo;上一页</a></li>
            </c:otherwise>
        </c:choose>
        <c:choose>
            <c:when test="${page.pageNum == page.pages }"/>
            <c:otherwise>
                <li><a href="${path}/base/searchOrder?page=${page.nextPage}&limit=${limit}">下一页&raquo;</a></li>
                <li><a href="${path}/base/searchOrder?page=${page.isLastPage}&limit=${limit}">最后页</a></li>
            </c:otherwise>
        </c:choose>
