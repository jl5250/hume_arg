...
<head>
    ...
    <script type="text/javascript">
        function changlimit() {
            ...
            location.href = "${path}/station/stationcouriers?page=1&limit=" + limit;
        }
    </script>
</head>
<body>
<div align="center">
    <c:choose>
        <c:when test="${page.pages == 0 }">
            <h2 style="color: red;">暂无雇佣的快递员</h2>
        </c:when>
        <c:otherwise>
            <table class="table table-hover">
                ...
                <c:forEach begin="0" step="1" items="${userlist}" var="list" varStatus="userlist">
                    ...
                    <tr>
                        ...
                        <td>${list.city}</td>
                        <td>${list.area}</td>
                        <td><a href="${path }/station/fire?courierId=${list.courierId}">解雇</a></td>
                    </tr>
                    </tbody>
                </c:forEach>
            </table>
            <ul class="pager">
                <c:choose>
                    ...
                    <c:otherwise>
                        <li><a href="${path}/station/stationcouriers?page=${page.firstPage}&limit=${limit}">首页</a></li>
                        <li><a href="${path}/station/stationcouriers?page=${page.prePage}&limit=${limit}">&laquo;上一页</a>
                        </li>
                    </c:otherwise>
                </c:choose>
                <c:choose>
                    ...
                    <c:otherwise>
                        <li>
                            <a href="${path}/station/stationcouriers?page=${page.nextPage}&limit=${limit}">下一页&raquo;</a>
                        </li>
                        <li><a href="${path}/station/stationcouriers?page=${page.lastPage}&limit=${limit}">最后页</a></li>
                    </c:otherwise>
                </c:choose>
